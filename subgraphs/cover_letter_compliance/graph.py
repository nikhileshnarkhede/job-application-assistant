"""
Cover Letter Compliance Subgraph Builder
...
def build_cover_letter_compliance_graph() -> StateGraph:
    """
    Build the Cover Letter Compliance subgraph.
    
    Returns:
        Compiled StateGraph for cover letter compliance validation
    """
    graph = StateGraph(CoverLetterComplianceState)
    
    # Add nodes
    graph.add_node("prepare_cover_letter_text", prepare_cover_letter_text)
    ...
    # Add edges
    graph.add_edge(START, "prepare_cover_letter_text")
    graph.add_edge("prepare_cover_letter_text", "validate_checklist")
...
def create_cover_letter_compliance_subgraph():
    """Create and return the compiled Cover Letter Compliance subgraph."""
    return build_cover_letter_compliance_graph()
...
def validate_cover_letter_compliance(
    cover_letter=None,
    cover_letter_text: str = None,
    structured_jd=None
) -> Dict[str, Any]:
    """
    Validate a cover letter against checklists and rubrics.
    
    Args:
        cover_letter: CoverLetter object from Cover Letter Generator
        cover_letter_text: Raw cover letter text (alternative to cover_letter)
        structured_jd: StructuredJD object for keyword matching (optional)
    
    Returns:
        Dict with compliance_report and validation results
    """
    graph = create_cover_letter_compliance_subgraph()
...
def get_cover_letter_compliance_summary(report: CoverLetterComplianceReport) -> str:
    """
    Generate a human-readable compliance summary.
    
    Args:
        report: CoverLetterComplianceReport object
    ...
    lines = [
        "=" * 60,
        "COVER LETTER COMPLIANCE REPORT",
        "=" * 60,
        "",
    ...
    # Rubric scores
    lines.append("📈 Rubric Scores (1-3 scale):")
    for score in report.rubric_categories:
        bar = "★" * score.score + "☆" * (3 - score.score)
        lines.append(f"   {score.category[:25]:25} [{bar}] {score.score}/3 (Weight: {int(score.weight*100)}%)")
    lines.append("")
...
def quick_cover_letter_compliance_check(cover_letter_text: str) -> Dict[str, Any]:
    """
    Quick compliance check returning just scores.
    
    Args:
        cover_letter_text: Raw cover letter text
    
    Returns:
        Dict with scores and pass/fail status
    """
    result = validate_cover_letter_compliance(cover_letter_text=cover_letter_text)
